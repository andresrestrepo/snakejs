...
const WIDTH_BOARD = 29
const HEIGHT_BOARD = 39
const BLOCK_SIZE = 20
...
function newBox(board) {
  const column = Math.floor(Math.random() * (WIDTH_BOARD - 0) + 0)
  const row = Math.floor(Math.random() * (HEIGHT_BOARD - 0) + 0)
  board[row][column] = 4
  score++
  document.getElementById('score').innerText = score
}

function paintTableScoreHistory() {
  SCORE_HISTORY.push(
    {
      score,
      "time": endTime()
  ...
  `
  SCORE_HISTORY.forEach((value, i) => {
    let trTag = `
    <tr>
      <td>${value.score}</td>
      <td>${value.time} seconds</td>
    </tr>
    `
    rows += trTag
  })

  document.getElementById('tbl_score_history').innerHTML = rows
}
...
function calculatePositionSnake() {
  ...
  if (tailPosition[0] < HEIGHT_BOARD) {
...
if (isMobile() || isTouchDevice()) {
  ...
  document.getElementById('desktop-keys').style.display = "block";
}
